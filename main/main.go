	...
	for {
		...
		case eventResp := <-eventPoll.Delete():
			go func(eventResp *poll.EventResponse) {
				...
				frm.Document <- flashsale.Delete(mc.AggCollection, &eventResp.Event)
			}(eventResp)
		...
		case eventResp := <-eventPoll.Insert():
			go func(eventResp *poll.EventResponse) {
				...
				frm.Document <- flashsale.Insert(mc.AggCollection, &eventResp.Event)
			}(eventResp)
		...
		case eventResp := <-eventPoll.Update():
			go func(eventResp *poll.EventResponse) {
				...
				frm.Document <- flashsale.Update(mc.AggCollection, &eventResp.Event)
			}(eventResp)
		}
	}
}
