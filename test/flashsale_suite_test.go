...
var _ = Describe("FlashsaleAggregate", func() {
	...
	BeforeSuite(func() {
		...
		marshalInv, err := json.Marshal(mockFlash)
		Expect(err).ToNot(HaveOccurred())
	...
	Describe("Flashsale Operations", func() {
		It("should insert record", func(done Done) {
			...
			msgCallback := func(msg *sarama.ConsumerMessage) bool {
				...
				if kr.UUID == mockEvent.UUID {
					...
					flash := &flashsale.Flashsale{}
					err = json.Unmarshal(kr.Result, flash)
					Expect(err).ToNot(HaveOccurred())

					if flash.ItemID == mockFlash.ItemID {
						mockFlash.ID = flash.ID
						Expect(flash).To(Equal(mockFlash))
						return true
					}
				}
				return false
			...
			Expect(err).ToNot(HaveOccurred())
			findResult, err := aggColl.FindOne(mockFlash)
			Expect(err).ToNot(HaveOccurred())
			findInv, assertOK := findResult.(*flashsale.Flashsale)
			Expect(assertOK).To(BeTrue())
			Expect(findInv).To(Equal(mockFlash))
		...
		// 			validationResp := &flashsale.SaleValidationResp{}
		// 			err = json.Unmarshal(event.Data, validationResp)
		// 			Expect(err).ToNot(HaveOccurred())
		// 			respItemID := validationResp.Result[0].ItemID
		// 			Expect(respItemID).To(Equal(mockFlash.ItemID))
		...
		It("should update record", func(done Done) {
			...
			mockFlash.Lot = "new-lot"
			mockFlash.UnsoldWeight = 500
			// Remove ObjectID because this is not passed from gateway
			mockID := mockFlash.ID
			mockFlash.ID = objectid.NilObjectID
		...
		It("should delete record", func(done Done) {
			...
			deleteArgs := map[string]interface{}{
				"flashsaleID": mockFlash.FlashsaleID,
			}
			...
			aggColl, err := loadAggCollection()
			Expect(err).ToNot(HaveOccurred())
			_, err = aggColl.FindOne(mockFlash)
